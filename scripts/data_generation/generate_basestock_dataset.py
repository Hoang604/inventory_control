"""
Generate Base-Stock Policy Dataset

Uses optimal parameters from grid search: z=[70,170,15]
"""
import os
...
# Optimal configs from grid search (profit: 346.11)
CONFIGS = [
    [70, 170, 15],
    [70, 170, 10],
    [80, 190, 10],
...
def generate_dataset(num_warehouses: int, days_per_warehouse: int, save_path: str) -> None:
    print(f"Generating Base-Stock Dataset")
    ...
    for warehouse_id in tqdm.tqdm(range(num_warehouses), desc="Warehouses"):
        ...
        policy = BaseStockPolicy(env, z=config)
...
if __name__ == "__main__":
    ...
    save_path = str(PROJECT_ROOT / "data" / "inv_management_basestock.pt")
