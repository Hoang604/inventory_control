"""
Generate Periodic Review (T,S) Policy Dataset

Uses optimal parameters from grid search: T=2, S=[100,200,50]
"""
import os
...
# Optimal configs from grid search (profit: 363.51)
CONFIGS = [
    (2, [100, 200, 50]),
    (3, [150, 250, 50]),
    (6, [200, 250, 50]),
...
def generate_dataset(num_warehouses: int, days_per_warehouse: int, save_path: str) -> None:
    print(f"Generating Periodic Review Dataset")
    ...
    for warehouse_id in tqdm.tqdm(range(num_warehouses), desc="Warehouses"):
        ...
        T, S = CONFIGS[warehouse_id % len(CONFIGS)]
...
if __name__ == "__main__":
    ...
    save_path = str(PROJECT_ROOT / "data" / "inv_management_periodic.pt")
