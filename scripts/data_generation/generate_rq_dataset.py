"""
Generate (R,Q) Fixed Quantity Policy Dataset

Uses optimal parameters from grid search: R=[80,200,0], Q=[20,20,5]
"""
import os
...
# Optimal configs from grid search (profit: 431.44)
CONFIGS = [
    [[80, 20], [200, 20], [0, 5]],
    [[80, 20], [200, 20], [0, 15]],
    [[80, 20], [200, 20], [0, 10]],
...
def generate_dataset(num_warehouses: int, days_per_warehouse: int, save_path: str) -> None:
    print(f"Generating (R,Q) Policy Dataset")
    ...
    for warehouse_id in tqdm.tqdm(range(num_warehouses), desc="Warehouses"):
        ...
        policy = RQPolicy(env, rq_params=config)
...
if __name__ == "__main__":
    ...
    save_path = str(PROJECT_ROOT / "data" / "inv_management_rq.pt")
